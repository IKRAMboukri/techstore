...
function renderDashboard() {
  ...
  // 1. Vérifier si on est en train de rendre
  if (isRendering) {
    console.log('Dashboard: déjà en cours de rendu');
    return;
  }
  
  // 2. Limiter à 1 appel par seconde maximum
  if (now - lastDashboardRender < 1000) { // Max 1 appel par seconde
    console.log('Dashboard: trop rapide, ignoré');
    return;
  }
  
  // 3. Vérifier si on est dans la section dashboard
  const dashboardSection = document.getElementById('dashboard');
  if (!dashboardSection || dashboardSection.classList.contains('d-none')) {
    console.log('Dashboard: section non visible');
    return;
  }
  
  // Marquer comme en cours de rendu
  isRendering = true;
...
// Fonction sécurisée pour appeler le dashboard
